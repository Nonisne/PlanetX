...
- [x] 使用独立模块实例测量冷启动，避免新增研究用例预热缓存导致性能预算漏检；修正后重新执行全量回归。
- [x] 恢复默认视口、关闭临时验收标签并停止本次创建的隔离服务，不影响现有用户服务。
- [x] 独立规格审查和代码审查，修复本次范围内的问题后复验；不提交、不创建 Git 仓库。

验收结论与证据：`docs/research-templates-report-2026-09-20.md`。全量 318 项中 316 通过、2 项已知启停旧文件名失败，穷举零跳过；冷启动预算另外通过延迟注入反例复核。
