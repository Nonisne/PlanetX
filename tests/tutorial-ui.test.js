...
test('tutorial research and declaration targets highlight only the expected topic or count', () => {
  ...
  state.game.tutorial = { ...state.game.tutorial, focus: 'theory', expected: { kind: 'research-declare', count: 1, objectTypes: [Obj.GAS_CLOUD], phaseId: 'phase-3' } };
  ...
  state.game.theoryTokensRemaining = { asteroid: 4, comet: 2, gasCloud: 2, dwarfPlanet: 1 };
  const actions = [];
  const api = { setUi(patch) { Object.assign(state.ui, patch); }, consoleAction(action) { actions.push(action); } };
  panel = renderActionPanel({ state, api });
  targets = elements(panel, element => element.tagName === 'button' && element.attributes?.['data-tutorial-target'] === 'type');
  assert.equal(targets.length, 1);
  assert.match(textOf(targets[0]), /气体云/);
  fire(targets[0], 'click');
  panel = renderActionPanel({ state, api });
  targets = elements(panel, element => element.tagName === 'button' && element.attributes?.['data-tutorial-target'] === 'count');
  assert.equal(targets.length, 1);
  fire(targets[0], 'click');
  assert.deepEqual(actions, [{ kind: 'research-declare', phaseId: 'phase-3', count: 1, objectTypes: [Obj.GAS_CLOUD] }]);
});
...
test('tutorial paper targets highlight the expected sector and object without submitting on behalf of the player', () => {
  ...
  state.game.research = { id: 'phase-3', sector: 3, myCount: 1, allDeclared: true, isMyPick: true, left: 1, nextType: Obj.GAS_CLOUD, orderNames: ['我', '领航员'], picks: [] };
  ...
  assert.match(textOf(panel), /这一篇已选定气体云/);
  assert.equal(elements(panel, element => element.tagName === 'button' && element.attributes?.['data-tutorial-target'] === 'type').length, 0);
